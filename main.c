...
struct min_deger tablo_deger(int,int,int[]); //m matrisi icin degerleri bulan fonksiyon
void yazdir(int [][7],int,int); // ekrana parantezler sirasini yazdiran fonksiyon

struct min_deger  //degerin bulundugu yeri de tutmak icin struct yapisi kullanildi
{
    int deger;
    int index;
};
...
int main()
{
    ...
    scanf("%d",&sayi); // kac adet matris olacagini sayi degiskeni tutar

    int dizi_boyut[2*sayi]; // her matrisin boyutu dizi_boyut adli dizisinde tutuluyor
    long int matris[50][50][50]; // her matrisi ardarda tutmak icin 3 boyutlu dizi kullanildi (carpmada kolaylýk saglar)
    long int matris_ek[50][50];
    printf("Her matris icin n x m boyutu girilmelidir.\n\n");

    printf("\n A1 in matrisi icin n(satiri) giriniz:");
    scanf("%d",&n);
    dizi_boyut[0]=n; // ilk eleman oldugu icin direk diziye atanir

    FILE *Dosya; // tum matrisleri tutan dosya yaratiliyor
    ...
    else
    {
        ...
        for(i=0; i<sayi; i++)
        {
            ...
            dizi_boyut[2*(i+1)-1]=m;
             // dosyaya nxm boyutu yazdirilir
            fprintf(Dosya,"\n A %d matrisi (%d x %d):\n",i+1,n,m);
            for(a=0; a<n; a++)
            {
                for(b=0; b<m; b++)
                {
                    // random ile matrisin degerler uretilir
                    rastgele=rand()%10;
             ...
             // bir  matrisin sutunu sonraki matrisin satiri olacagi icin kullanicidan alinmaz
            if(i+2<=sayi)
            {
    ...
    {
        //matrisin carpimi gerceklesiyor
...

        {
            for(b=0; b<dizi_boyut[3]; b++)
            {
                ...
                matris_ek[a][b]=sum; // gecici olarak ek matrise atanir
    ...
    int j,k,g;
    // Dinamik progrmalama ile carpimi
    ...
    int p[sayi+1]; // p dizisi boyutları tutar
    ...
    {   // m matrisi yazdirilir hem ekrana hem dosyaya
         fprintf(dosya3,"\n\n m matrisi:\n");
        printf("\n\n m matrisi:\n");
        int u;
        fprintf(dosya3,"\n");
...
struct min_deger tablo_deger(int i, int j,int p[])
{
    ...
    else
    {
        ...
        for(k=i; k<j; k++)
        {
               // formulu ile carpim sayisi hesaplanir
            z=tablo_deger(i,k,p);
...
void yazdir( int matris[][7], int i, int j) // burda boyutu olarak 7 belirtildi bu demekki sadece 6 tane matris icin calisir
// daha farkli sayilarda matrisi carpmak icin burdan sayi 7 yerine istediginiz sayi yaziniz
{
